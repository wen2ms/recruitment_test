...
1. 物理层：通过链路进行比特传输。
2. 数据链路层：将数据封装成帧和差错控制，协议包括以太网网`Ethernet`，`MAC`。
3. 网络层：路由。例如`IP`协议。
...
3. 第三次挥手：服务端不再发送数据，发送`FIN seq = v`报文段，进入`LAST_WAIT`状态。
4. 第四次挥手：客户端接收到`FIN`报文段后，回复一个确认报文段`ACK ack = v + 1`，然后进入`TIME_WAIT`状态，等待`2MSL`时间，来确保`ACK`报文段。服务端接收到`ACK`报文段后关闭连接，在等待完成后也关闭连接。至此，双方断开连接成功。这里`2MSL  (Maximum Segment Lifetime)`代表TCP报文段在网络中允许存活的最大时间的两倍，客户端发送的`ACK`最长`1MSL`到达服务端，同时服务端重传的`FIN`最长也是`1MSL`，因此`2MSL`保证了这一点。
...
TCP是面向连接的流式传输协议，它的传输单位是报文段，但应用层看到的只是字节流，没有规定消息边界，因此接受方在读取的时候会将多个数据包合并在一起。
...
3. 快重传机制：接收方不要等到自己发送数据的时候才发送`ACK`报文段，而是接收到数据的时候就直接发送`ACK`报文段。其次就是当收到失序的报文段要发出已经收到的报文段的重复确认`ACK`，最后当收到三个重复的`ACK`报文段后，立刻重传，不再等候超时重传计时器超时。
