...
7. 智能指针不能替代裸指针，不适合栈上的对象或者不需要动态分配的对象，`get`的使用也可能导致内存管理问题。同时智能指针会自动管理内存，而裸指针不需要负责销毁。
...
- 构造函数不会调用子类的虚函数。vptr的初始化和对象的初始化是一致的，当子类对象构造时，先调用父类的构造函数，再调用成员对象的构造函数，最后调用自己的构造函数，当调用父类的构造函数的时候`vptr`指向是父类的虚函数。

- 构造函数不能是虚函数，`vtable`需要将对象完全构造好之后才能确定，而构造函数如果是虚函数，就形成逻辑上的错误。

- 多重继承下，对象内部就会有多个`vptr`，每个`vptr`指向不同的`vtable`。
 ...
 - 虚函数不建议带默认参数。因为默认参数是在编译期绑定的，而虚函数是在运行时确定要调用哪一个函数，因此会出现实际调用的虚函数的参数和默认参数不一致的情况。
...
当类中有`const`成员或者引用成员的时候，编译器不会自动生成移动构造函数和移动赋值运算符，因为`const`成员需要初始化，同时之后不能被修改，引用也是必须要初始化，不能重新赋值引用。
