...
一个写请求的流程是：1. 写请求到达。2. 生成`Redo Log`和`Undo Log`，然后调用`write`将`Redo Log`从数据库的内存缓冲池写入到内核缓冲区中。3. 修改内存缓冲池中的数据页，产生脏页。下一步就是根据不同的情况，一种情况是客户端主动发出`ROLLBACK`或者事务中途失败，另一种是客户端直接`COMMIT`。
