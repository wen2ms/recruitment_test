...
先写`WAL`并持久化首先保证安全性，其次先修改内存页保证高性能，最终异步刷盘减少了`I/O`瓶颈。
...
当数据库意外崩溃的时候，内存中未刷盘的数据可能丢失，需要利用`WAL`来恢复。当数据库启动时首先会扫描`WAL`，在`Redo`阶段`Redo Log`保证已提交的事务重新落盘，`Undo`阶段`Undo Log`会将未提交事务的被脏页污染了的磁盘数据回滚，最终数据库恢复到一致状态。
