...
将多个目标文件`.o`的同类段合并，形成一个可执行文件的段布局，每个段被赋予一个虚拟地址，会在`ELF`文件中会定义
...
程序加载会从`ELF (Excutable and Linkable Format)`文件中读取`.text`的机器码，`.rodata`的常量和`.data`的初始值，`.bss`只分配虚拟页，值全部清零。运行时将段映射到进程的虚拟地址空间，最后CPU只会执行`.text`段中的代码（机器码），数据存储在其他段中。
