...
在追求实时性的场景下`UDP`更加好用，例如语音通话，视频会议，在线游戏，这些应用都可以允许丢包，但对最新状态的实时性要求更高，而`UDP`的无连接和轻量的特点就能满足。还有一个应用场景是`DNS`查询，它也不需要像`TCP`中连接的管理，`UDP`更加简单快速，它可以单播请求多个域名服务器，即使丢了一个发送给域名服务器的包也不影响其他服务器，甚至即使全部失败也可以重新递归查询。
